#!/usr/bin/env bash
# Runs ONCE at first boot, not at image-build time: the runner auto-update is
# disabled for ephemeral/JIT runners, so a version pinned into the image goes
# stale within weeks. Always fetch the current release at deploy time.
#
# Parameterized via the systemd unit's environment:
#   RUNNER_ARCH   release arch slug: x64 (x86_64) or arm64 (aarch64)
#   RUNNER_OWNER  owner for the unpacked tree (username or uid)
#   RUNNER_GROUP  group for the unpacked tree (group name or gid)
set -euo pipefail
